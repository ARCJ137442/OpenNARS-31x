...
public class TemporalRules {
    ...
    public static Task temporalInduction(Sentence s1, Sentence s2, Task currentTask, Sentence belief,
            boolean sequenceInduction, Memory memory) {

        // 如果有语句的真值为空，或者有语句不是判断句，或者有语句是永恒的，则无法做时间归纳
        if ((s1.getTruth() == null) || (s2.getTruth() == null) || s1.getPunctuation() != Symbols.JUDGMENT_MARK
                || s2.getPunctuation() != Symbols.JUDGMENT_MARK
        ...
        final BudgetValue budget4 = BudgetFunctions.forward(truth4, memory); // this one is sequence in sequenceBag, no
                                                                             // need to reduce here
