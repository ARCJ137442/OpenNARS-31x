...
public abstract class EventBuffer extends Buffer<Task> {
    ...
    /**
     * Process Anticipation, go over the current expectation list, if the
     * expectation of the task is higher than
    ...
    public void processAnticipation(Task task) {
        ...
        // No additional implementation is required for the positive evidence because if
        // the expected task happens
    ...
    protected ArrayList<Task> eventInference(Task newEvent, boolean allowOverlap) {
        ...
        if (index < 1)
            return null;
        // some of the sequence conjunction has the same occurence time with the
        // previous event
        // for example, if a,b in the list, (&/, a, b) is generated, and in the list, it
        // looks like
        // a, b, (&/, a, b) so when c come into the list, c should do the temporal
        // induction with
        ...
        // Go through the entire list to find out all the event that could do the
        // tempora inference with
        // the task
        for (Task t : sequenceList) {
            ...
            if (t.getContent() == null || t.getBudget() == null
                ...
                continue;
            // if evidence overlap is not allowed but overlao is found, skip the current
            // event
            if (!allowOverlap && Stamp.baseOverlap(newEvent.getStamp(), t.getStamp())) {
                continue;
            }
            // if the difference of occurrence time is great than the duration of the
            // buffer, skip
            ...
            // check if the event has the same occurrence time with the one happens before
            // the task
            ...
            if (newEvent.getStamp().getOccurrenceTime() < t.getStamp().getOccurrenceTime())
                newTask = TemporalRules.temporalInduction(newEvent.getSentence(), t.getSentence(), newEvent,
                        t.getSentence(), sequenceInduction, this.getMemory());
            else
                newTask = TemporalRules.temporalInduction(t.getSentence(), newEvent.getSentence(), newEvent,
                        t.getSentence(), sequenceInduction, this.getMemory());
            if (newTask != null)
                list.add(newTask);
    ...
    public void clearExpiredAnticipation() {
        ...
        // Go through the entire expectation list, if the expectation is expired, which
        // the current time
        ...
        // Since the expected event didn't happen, generate a negative evidence to the
        // implication
        // the negative evidence is for the implication which generated the expectation,
        // witht the same
        // time interval. It doesn't matter if the expected event happens after the
        // expected time, since another positive
        // evidence will be generated with the new time interval by the temporal
        // induction
        TruthValue negaTruth = new TruthValue(0.0f, Parameters.DEFAULT_JUDGMENT_CONFIDENCE);
        // float quality = (negaTruth == null) ? 1 :
        // BudgetFunctions.truthToQuality(negaTruth);
        // BudgetValue budget = new BudgetValue(Parameters.DEFAULT_EVENT_PRIORITY,
        // Parameters.DEFAULT_EVENT_DURABILITY, quality);

        // go over the failed anticipatio nlist and generate negative evidence and do
        // the revision directly
        for (int i = 0; i < failedAnticipation.size(); i++) {

            Stamp stamp = new Stamp(this.getMemory().getTime());
            stamp.setEternal();
            Sentence sentence = new Sentence(failedAnticipation.get(i).getRelation(), Symbols.JUDGMENT_MARK, negaTruth,
                    stamp);
            ...
            // this.getMemory().generalInfoReport("Failed: " +
            // sentence.getContent().getName());
    ...
    protected int findIntertIndexInSequenceList(Task task) {
        ...
        // go over the entire list from the tail and find the first one which the
        // occurrence less than the occurrence time
    ...
    public boolean putInSequenceList(Task task, long time) {
        ...
        // if successfully insert into the priority list, insert the event to the
        // sequence list
        if (putIn(task, this.getItemTable(), capacity()) && !task.isEternal() && !task.getSentence().isGoal()
                && task.isInput()) {
            ...
            if (sequenceList.isEmpty()) {
                ...
                if (sequenceList.size() < capacity()) {
                    ...
                    // if the sequnce list is full, check if the oldest one can be remove and insert
                    // the new one into it
    ...
    public void generateExpectation(Task task) {
        ...
        // if the difference between the ot(occurrence time) and the current time is
        // greater than
        // the duration, return
        if (Math.abs(task.getStamp().getOccurrenceTime()
                - this.getMemory().getTime()) > Parameters.DURATION_FOR_GLOBAL_BUFFER)
            return;
            ...
            for (Concept anticipation : concept.getAnticipationList()) {
                ...
                if (belief.getContent() instanceof Implication) {
                    ...
                    if (belief.getTemporalOrder() == TemporalRules.ORDER_FORWARD) {
                        ...
                        Task expectEvent = SyllogisticRules.detachment(belief, task.getSentence(), 0, this.getMemory(),
                                true);

                        if (expectEvent == null)
                            return;

                        Anticipation expectation = new Anticipation(expectEvent, belief.getContent(),
                                expectEvent.getStamp().getOccurrenceTime(), task.getStamp().getOccurrenceTime());
                        ...
                        if (addToExpectationList(expectation)) {
                             ...
                             * //this.getMemory().generalInfoReport("Anticipate " +
                             * expectation.getTask().getName() + " to happen at " +
                             * expectEvent.getStamp().getOccurrenceTime());
                             * this.getMemory().generalInfoReport("Anticipate " +
                             * expectation.getTask().getName()
                             ...
                             * this.getMemory().generalInfoReport("Relation: " +
                             * belief.getContent().getName() + " " + belief.getTruth().getExpectation());
                             * }
                             */
                            ...
                            if (c != null && !c.getDesires().isEmpty()) {
                                ...
                                if (expectEvent.getSentence().getTruth().getExpectation() < 0.45) {
                                    ...
                                    if (c.getDesires().get(0).getSentence().getTruth().getExpectation() > 0.5) {
                                        ...
                                        /*
                                         * this.getMemory().
                                         * generalInfoReport("NARS is feeling fear. Because NARS desires "
                                         * + expectEvent.getName() + "\nto happen, with expectation "
                                         * + c.getDesires().get(0).getSentence().getTruth().getExpectation()
                                         * + ",\n but NARS anticipates " + expectEvent.getName() +
                                         * "will not happen, with expectation "
                                         * + expectEvent.getSentence().getTruth().getExpectation());
                                         */

                                        float weight = expectEvent.getPriority();
                                        float fear = Math.abs(expectEvent.getSentence().getTruth().getExpectation()
                                                - c.getDesires().get(0).getSentence().getTruth().getExpectation());
                                        float timeDiff = Math.abs(expectEvent.getStamp().getOccurrenceTime()
                                                - this.getMemory().getTime());
                                        float timeImpact = 1 - timeDiff / 11;

                                        c.getDesires().get(0).setBestSolution(expectEvent.getSentence());

                                        float newPriority = UtilityFunctions.or(c.getPriority(), weight, fear,
                                                timeImpact);
                                        float newDurability = UtilityFunctions.or(c.getDurability(), weight, fear,
                                                timeImpact);
                                    ...
                                    if (c.getDesires().get(0).getSentence().getTruth().getExpectation() <= 0.5) {
                                        ...
                                        /*
                                         * this.getMemory().
                                         * generalInfoReport("NARS is feeling fear. Because NARS does not desire "
                                         * + expectEvent.getName() + "\n to happen, with expectation "
                                         * + c.getDesires().get(0).getSentence().getTruth().getExpectation()
                                         * + ",\n but NARS anticipates " + expectEvent.getName() +
                                         * " will happen, with expectation "
                                         * + expectEvent.getSentence().getTruth().getExpectation());
                                         */
                                        ...
                                        float newPriority = UtilityFunctions.or(c1.getPriority(), weight, fear,
                                                timeImpact);
                                        float newDurability = UtilityFunctions.or(c1.getDurability(), weight, fear,
                                                timeImpact);

                                        BudgetValue buget = new BudgetValue(newPriority, newDurability,
                                                c1.getQuality());
                        ...
                        Task expectEvent = SyllogisticRules.detachment(belief, task.getSentence(), 1, this.getMemory(),
                                true);
                        ...
                        Anticipation expectation = new Anticipation(expectEvent, belief.getContent(),
                                expectEvent.getStamp().getOccurrenceTime(), task.getStamp().getOccurrenceTime());
                        if (addToExpectationList(expectation)) {
                            // System.out.println("Anticipate " + expectation.getTask().getName() + " to
                            // happen at " + expectEvent.getStamp().getOccurrenceTime());
                            /*
                             * this.getMemory().generalInfoReport("Anticipate " +
                             * expectation.getTask().getName()
    ...
    public boolean addToExpectationList(Anticipation expectation) {
        ...
        if (expectationList.isEmpty()) {
            ...
            if (expectationList.size() < anticipationCapacity()) {
                if (expectation.getTask().getPriority() < expectationList.get(expectationList.size() - 1).getTask()
                        .getPriority()) {
                    expectationList.add(expectation);
                    return true;
                } else {
    ...
    public boolean checkDuplicates(Task task, ArrayList<Task> list) {
        ...
        for (Task t : list) {
            ...
            if (task.getName().equals(t.getName())
                    && task.getStamp().getOccurrenceTime() == t.getStamp().getOccurrenceTime())
                return true;
    ...
    /**
     * Clear the expired items, if the difference of the put in time and the current
     * time
     * greater than the duration which mean stay in the buffer for too long, remove
     * it
     */
    ...
    protected Task takeOut() {
        ...
        // if the item table is not null, return the first one which the one with
        // highest priority
